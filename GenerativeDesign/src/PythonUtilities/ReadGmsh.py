...
def OrganiseNodesIn1DMesh(nodes, elements):
    ...
    if closed:
        ...
        if len(nodei) == 1:
            ...
            nodei = LeastFrequentElementsInList(nodei)
            if len(nodei) == 1:
                nodei = nodei[0]
            elif len(nodei) == 2:
                nodei = nodei[0]
...
def Remesh(grpNamList, filNamList, lcList, faceNams, modelIn,
    ...
    if geoType == 'std':
        ...
        raise("Error: unavailable geoType: " + geoType)
    ...
    for k1 in range(numFaces):
        ...
        cntrs = contours[k1]
        for k2 in range(numContours[k1]):
            splns[k2] = geo.addSpline(cntrs[k2])
        splines[k1] = splns
    # Closed curve loops
    closedContours = [None]*numFaces
    for k1 in range(numFaces):
        splns = splines[k1]
        closedContours[k1] = geo.addCurveLoop(splns)
    # Faces
    faces = [None]*numFaces
    for k1 in range(numFaces):
        face = faces[k1]
        faces[k1] = geo.addPlaneSurface([closedContours[k1]])
        if recombine[k1]:
            geo.mesh.setRecombine(2, faces[k1])
    # Synchronize
    geo.synchronize()
...
def RemeshLine(model, recoFile, lc):
    ...
    for node in nds:
        ...
        yi = node[1]
        zi = node[2]
        points.append(geo.addPoint(xi, yi, zi, lc))
    # Make line
    line = geo.addSpline(points)
    return points, line
# }}}
